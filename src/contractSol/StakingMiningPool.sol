...
contract StakingMiningPool is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt;     // rewardDebt指用户不可提取的那部分激励
    }

    // Info of each pool.
    struct PoolInfo {
        IERC20 stakedToken;       // 需要抵押的代币
        IERC20 minedToken;        // 被挖出的代币
        uint256 stakedAmount;        // 本池子已经抵押的代币数量，需要单独统计，因为代币只能打给整个合约，而不能给本池子
        uint256 minedAmount;         // 本池子可挖的代币数量，需要单独统计，因为代币只能打给整个合约，而不能给本池子
    ...
    function addPool(IERC20 _stakedToken, IERC20 _minedToken, address _fromAccountOfStakedToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 tokenReward = pool.minedAmount;   // 本池子中剩余的可挖矿代币数量
            accTokenPerShare = accTokenPerShare.add(tokenReward.mul(1e12).div(pool.stakedAmount));   // 此处乘以1e12，在下面会除以1e12
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply = pool.stakedAmount;   // 本池子占有的LP数量
        if (lpSupply == 0) {
            pool.lastRewardBlock = block.number;
            return;
        }
        uint256 tokenReward = pool.minedAmount;   // 计算本池子可获得的新的token激励
    ...
    function depositStakeToken(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfoList[_pid];            // 获取挖矿池
    ...
    function supplyMinedToken(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfoList[_pid];  
        pool.minedToken.transferFrom(msg.sender, address(this), _amount);  // 将msg.sender提供的矿转移到本合约;需要先approve：msg.sender -> this
        pool.minedAmount = pool.minedAmount.add(_amount);        // 将挖出的矿计入到某矿池中
        pool.totalMinedAmount = pool.totalMinedAmount.add(_amount);
    }
